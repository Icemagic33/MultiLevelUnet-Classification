...
model = VGG16(num_classes=len(labels)).to(device)
print(model)
...
for epoch in range(num_epochs):
    ...
    if val_accuracy > best_accuracy:
        ...
        torch.save(model.state_dict(), 'vgg_es_best_model.pth')
        no_improvement_epochs = 0
    else:
...
model.load_state_dict(torch.load('vgg_es_best_model.pth'))
model.eval()
...
plt.tight_layout()
plt.savefig('vgg_es_confusion_matrix_classification_report.png')
plt.show()
...
plt.tight_layout()
plt.savefig('vgg_es_loss_accuracy_plot.png')
plt.show()
