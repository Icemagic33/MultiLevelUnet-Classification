...
# Identify Minority Classes: Moderate and Severe
def is_minority_class(label):
    # Considering that Moderate and Severe are minority classes
    return label[1] == 1 or label[2] == 1


# Custom Dataset Class
class CustomImageDataset(Dataset):
    def __init__(self, csv_file, transform_basic, transform_aug):
        self.data = pd.read_csv(csv_file)  # Load the CSV file containing image paths and labels
        self.transform_basic = transform_basic  # Store the basic transformations
        self.transform_aug = transform_aug  # Store the augmentations
    ...
    def __getitem__(self, idx):
        ...
        # Load the labels
        label = self.data.iloc[idx, 3:].values.astype(float)
        
        # Apply transformations conditionally
        if is_minority_class(label):
...
# Load dataset
dataset = CustomImageDataset(csv_file, transform_basic, transform_aug)
...
class SimpleCNN(nn.Module):
    def __init__(self, num_classes=75 * 3):  # 75 conditions, each with 3 classes (normal/mild, moderate, severe)
...
num_classes = 25 * 3  # 25 conditions, each with 3 classes
